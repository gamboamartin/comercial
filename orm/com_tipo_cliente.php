...
class com_tipo_cliente extends _modelo_parent_sin_codigo
{
    ...
    {
        $validacion = $this->validacion(datos: $this->registro);
        if (errores::$error) {
    ...
    public function validacion(array $datos)
    {

        if (strlen($datos['descripcion']) < 3) {
            $mensaje_error = sprintf("Error el tipo de cliente '%s' debe tener como minimo 3 caracteres", $datos['descripcion']);
    ...
    {
        $validacion = $this->validacion(datos: $registro);
        if (errores::$error) {
            return $this->error->error(mensaje: 'Error de validacion', data: $validacion);
