...
class com_contacto extends _modelo_parent{
    ...
    public function alta_bd(array $keys_integra_ds = array('codigo', 'descripcion')): array|stdClass
    {
        ...
        $filtro['com_contacto.descripcion'] = $this->registro['descripcion'];
        $existe = (new com_contacto(link: $this->link))->existe(filtro: $filtro);
        if(errores::$error){
            return $this->error->error(mensaje: 'Error al validar',data:  $existe);
        }

        if($existe){
            $r_com_contacto = (new com_contacto(link: $this->link))->filtro_and(filtro: $filtro);
            if(errores::$error){
                return $this->error->error(mensaje: 'Error al obtener registro',data:  $r_com_contacto);
            }
            if($r_com_contacto->n_registros === 0){
                return $this->error->error(mensaje: 'Error no existe registro',data:  $r_com_contacto);
            }
            if($r_com_contacto->n_registros > 1){
                return $this->error->error(mensaje: 'Error existe mas de un registro',data:  $r_com_contacto);
            }

            $this->registro_id = $r_com_contacto->registros[0]['com_contacto_id'];
            ...
            $r_alta_bd->registro = $r_com_contacto->registros[0];
            $r_alta_bd->registro_obj = (object)$r_com_contacto->registros[0];
